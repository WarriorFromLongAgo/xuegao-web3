...
use crate::business::model::doo::balance_do::BalanceDo;
...
pub async fn create_batch_addresses_test(pool: &PgPool) {
    create_batch_addresses(pool, 10, AddressTypeEnum::UserAddress, CoinTypeEnum::Ether).await;
    create_batch_addresses(pool, 3, AddressTypeEnum::HotWallet, CoinTypeEnum::Ether).await;
    create_batch_addresses(pool, 3, AddressTypeEnum::CollectionWallet, CoinTypeEnum::Ether).await;
    create_batch_addresses(pool, 3, AddressTypeEnum::ColdWallet, CoinTypeEnum::Ether).await;
    return;
}
