...
export async function signHash(hash: Uint8Array, privKey: Uint8Array) {
    const sig = await secp256k1.signAsync(hash, privKey);
...
export function recoverPublicKey(hash: Uint8Array | string, sig: Uint8Array) {
    ...
    const secpSig = secp256k1.Signature.fromCompact(sig.slice(0, -1)).addRecoveryBit(
        recoveryBit[0],
    );
...
export function verify(
    ...
    return secp256k1.verify(sig.slice(0, -1), hash, publicKey);
}
